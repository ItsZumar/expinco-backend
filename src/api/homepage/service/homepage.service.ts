...
export const homepageService = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  const currData = new Date();
  const currYear = currData.getFullYear();
  const currMonth = currData.getMonth();
  const currDate = currData.getDate();
  ...
  let totalAmount = await Transaction.aggregate([
    {
      $match: {
        ...
        //   $lte: new Date(currYear, currMonth, currDate),
        // },
      },
    },
    {
